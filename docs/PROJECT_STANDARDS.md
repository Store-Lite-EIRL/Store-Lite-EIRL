...
- **Archivos de Lógica/Utilidades**: `kebab-case` o `camelCase` (consistente con el entorno). *Corregir archivos como `CreateBusiness.ts` a `create-business.ts`*.
...
*Nota: Esta guía es dinámica. Si encontrás un patrón mejor, proponelo, discutimos y actualizamos.*
