...
// =====================================================
// ComplaintForm — Platform Libro de Reclamaciones form
// =====================================================
// Public platform form (no store slug). Submits via the
// slug-free submitPlatformComplaint server action. Copy is
// neutral professional Spanish (Peru) per DS 011-2011-PCM.
// =====================================================
  ...
  const handleSubmit = () => {
    startTransition(async () => {
      ...
      if (Object.keys(errors).length > 0) {
        ...
        setFeedback({
          open: true,
          description: 'Corrija los errores en el formulario.',
          color: 'error',
        });
        return;
      }

      const { submitPlatformComplaint } = await import('@/features/legal/actions');
      const res = await submitPlatformComplaint({
      ...
      if (res.success) {
        posthog.capture('platform_complaint_submitted', {
    ...
    return (
      <Card variant="outlined" style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '0.875rem', color: 'var(--md-sys-color-on-surface-variant)' }}>
          La empresa tiene 15 días hábiles para responder. Si no recibe respuesta, puede escalar el
            ...
            <div style={{ display: 'flex', gap: '1.5rem' }}>
              ...
              >
                <input
                  ...
                  onChange={() => setFormData((prev) => ({ ...prev, consumerDocumentType: 'dni' }))}
              ...
              >
                <input
                  ...
                  onChange={() => setFormData((prev) => ({ ...prev, consumerDocumentType: 'ce' }))}
                  style={{ accentColor: 'var(--md-sys-color-primary)' }}
                />
